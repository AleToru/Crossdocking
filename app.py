        ...
        for i in I:
            for j in J:
                for k in K:
                    val = value(x[(i,j,k)])
                    if val and val > 0.01:
                        fin_descarga_i = value(a[i]) + tiempo_descarga(i, K, r)
                        inicio_carga_j = value(d[j])
                        if inicio_carga_j < fin_descarga_i:
                            tiempo_extra = fin_descarga_i - inicio_carga_j
                            tiempo_extra_total += tiempo_extra * val
                            st.write(f"Camión entrante {i} → Camión saliente {j} | Producto {k}: {val:.0f} unidades ⚠️ VÍA ALMACENAMIENTO TEMPORAL")
                        else:
                            st.write(f"Camión entrante {i} → Camión saliente {j} | Producto {k}: {val:.0f} unidades ✅ DIRECTO")

        st.subheader("Análisis del almacenamiento temporal")
        if tiempo_extra_total > 0:
            st.warning(f"Tiempo extra generado por almacenamiento temporal: {tiempo_extra_total:.0f} minutos·unidad")
            st.write("El almacenamiento temporal genera congestión y movimientos innecesarios que aumentan el makespan.")
        else:
            st.success("No se usó almacenamiento temporal en esta solución.")